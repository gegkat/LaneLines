...
I verified that my perspective transform was working as expected by testing on an image with straight parallel lane lines and verifying that the lines appear parallel and vertical in the warped image. Here is an example of a warped test image with parallel vertical lines. This image has also been converted to gray scale. 
...
The margin method is used if valid lane lines were identified recently in the video stream. In this method indices within 100 pixels of the previous lane line are considered to be lane-line pixels. This is executed in lines 325 to 328 of `Lane.py`. 
...
at a point y is   
<code>curvature = [(1 + (2Ay + B)<sup>2</sup>)<sup>1.5</sup>]/|2C|</code>  
...
<code>curvature = (1 + B<sup>2</sup>)<sup>1.5</sup>/|2C|</code>   
